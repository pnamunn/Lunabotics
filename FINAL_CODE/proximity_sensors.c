/*
PD6 is Timer 0 Compare Match A Event				Drive train motor[s]L
PD5 is Timer 0 Compare Match B Event				Drive train motor[s]R

PB1 is Timer 1 Compare Match A Event				Deposition Bin deploy/retract
PB0 is Timer 1 Compare Match B Event				Deposition Bin doors:open/shut

PB3 is Timer 2 Compare Match A Event				Bucket Ladder deploy/retract
PD3 is Timer 2 Compare Match B Event				Bucket Ladder Chain start/stop/rvs


Author:												Stuart Pollmann
Program:											Stu-Bot Embedded
Application:										Robot Motions/Sensor reads
Use Case:											Brush DC Motor Control 
Functionality:										
Goal:												D-Pad implementation, then 
													joystick x/y axis differential
													drive train steering UART serial
													Sensor ADC Read channels multiplexed
													for multiple sensors, request/acknowledge
													for communication watchdog										
...
{
	DDRD	&= ~(1<<2);							//For Proximity
	EICRA	|= (1<<ISC00);						//Vector Block
	EICRA	&= ~(1<<ISC01);
	EIMSK	|= (1<<INT0);						//D7 for North interrupt
...
//void setSpd(uint8_t x, uint8_t y)				//Speed Set Still being developed
void setSpd()
{
	/*
	...
	*/
	...
	else
	{
		...
		OCR2B=0;
												...
												//ADC developing Function
/*
ISR(ADC_vect)									//no longer using
{												//keep when ready for sensors
	static	uint8_t	x = 0;
	static	uint8_t y = 0;
...
ISR(USART_RX_vect)
{
	...
	zyn4Scoobz = zynOverDose;				//heel scooby to not doo
...
/*
NOT TESTED
// Controller Scaled protocol

#define stUART_start_byte 0x02
#define stUART_end_byte 0x03

volatile uint8_t titanicPacket[JoyStickSize]
volatile uint8_t phase = 0
volatile uint8_t noCrashEven = 0;
...
ISR(USART_RX_vect)
{
		...
		if (packetIndex == 0 && receivedByte == START_BYTE) 
		{
			packet[phase++] = receivedByte;
		} 
		else if (packetIndex > 0 && packetIndex < PACKET_SIZE - 1) '
		{
			packet[phase++] = receivedByte;
		} 
		else if (packetIndex == PACKET_SIZE - 1 && receivedByte == END_BYTE) 
		{
			packet[phase] = receivedByte;
			noCrashEven = 1;
			phase = 0;				// Reset for next packet
		} 
		else 
		{
		
			phase = 0;				//Error reset state machine
		}
	
}

									//if noCrashEven, call function
									//not sure whether out or in
									//ISR
*/
